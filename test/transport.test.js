...
describe("http-transport:", () => {
  ...
  describe("addMessageListener:", () => {
    ...
    it("should catch invalid routingKey params", () => {
      return transport
        .addMessageListener(44444, (msg, correlationId, initiator) => {
          return Promise.resolve({
            result: `Received ${JSON.stringify(msg)}, ${correlationId}, ${initiator}`,
          });
        })
    ...
    it("should register a working get callback", () => {
      return transport
        .addMessageListener("bob", (msg, correlationId, initiator) => {
          return Promise.resolve({
            result: `Received ${JSON.stringify(msg)}, ${correlationId}, ${initiator}`,
          });
        })
          ...
          supertest(transport.app)
            ...
            .then((response) => {
              ...
              response.body.result.should.equal(
                'Received {"testParam":"5"}, testCorrelationId, testInitiator'
              );
            })
        );
    });
        ...
        .then(() =>
          supertest(transport.app)
            ...
            .then((response) => {
              // eslint-disable-line max-nested-callbacks
              response.body.result.should.equal(
                'Received {"postParam1":"test value"}, testCorrelationId, testInitiator'
              );
            })
        );
    });
  ...
  describe("request:", () => {
    ...
    beforeEach(() => {
      ...
      listenerTransport = new HTTPTransport();
      listenerTransport
        .addMessageListener("bob", (msg) => {
          return Promise.resolve({ message: `${msg.message}, bob!` });
        })
        .then(() =>
          listenerTransport.addMessageListener("steve", (msg) => {
            // eslint-disable-line max-nested-callbacks
            if (!msg.message) {
              return Promise.reject(
                new errors.InvalidMessageError('Missing required parameter "message"')
              );
            }
    ...
    it("should catch invalid routingKey params", () => {
      return transport
        .request(35353535, { message: "hello" }, { host: "localhost", port: 3000 })
        .then(() => {
          throw new Error("Failed to catch invalid input.");
        })
        .catch((err) => {
    ...
    it("should catch invalid correlationId params", () => {
      return transport
        .request("bob", {}, { correlationId: 44444, host: "localhost", port: 3000 })
        .then(() => {
          throw new Error("Failed to catch invalid input.");
        })
        .catch((err) => {
    ...
    it("should catch invalid initiator params", () => {
      return transport
        .request("bob", {}, { initiator: 44444, host: "localhost", port: 3000 })
        .then(() => {
          throw new Error("Failed to catch invalid input.");
        })
        .catch((err) => {
    ...
    it("should resolve to an invalid message error if the message is invalid", () => {
      return transport.request("steve", {}, { host: "localhost", port: 3000 }).catch((err) => {
    ...
    it("should resolve to an invalid message error if the user is unauthorized", () => {
      return transport.request("steveU", {}, { host: "localhost", port: 3000 }).catch((err) => {
    ...
    it("should resolve to a forbidden error if the user is forbidden", () => {
      return transport.request("steveF", {}, { host: "localhost", port: 3000 }).catch((err) => {
    ...
    it("should resolve to a not found error if need be", () => {
      return transport
        .request("steveNotFound", {}, { host: "localhost", port: 3000 })
        .catch((err) => {
    ...
    it("should resolve to a general processing error if a general error occurs", () => {
      return transport.request("dale", {}, { host: "localhost", port: 3000 }).catch((err) => {
